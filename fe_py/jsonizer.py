...
def process_footer(footer):
	...
	footer = footer.split('/')
	print('"total_marks":"' + footer[0] + '",')

def process_subjects(subjects):
	print("[")
	for subject in subjects:
		leading, trailling = subject[:36], subject[36:]
...
def process_header(header):
	...
	seat_num, *student_name, mom_name = header[0].split()
	seat_num, student_name, mom_name = seat_num.strip(), \
	join(student_name).strip(), mom_name.strip()
